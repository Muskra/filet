...
func Catch(grid [][]Cell, rules Rules) ([][]Cell, error) {
	...
	for i, line := range grid {
		for j := range line {
			...
			grid[i][j].State, err = actualCellState(coo, rules, grid)
			if err != nil {
			...
			case true:
				err := nextGeneration(grid, coo, realTargetLocations, rules.TargetValues.TargetIfAlive, rules.RuleSet[:])
				if err != nil {
					return nil, fmt.Errorf("< ! > Filet, CatchError: %s\n", err)
				}
			case false:
				err := nextGeneration(grid, coo, realTargetLocations, rules.TargetValues.TargetIfDead, rules.RuleSet[:])
				if err != nil {
					return nil, fmt.Errorf("< ! > Filet, CatchError: %s\n", err)
				}
			}
